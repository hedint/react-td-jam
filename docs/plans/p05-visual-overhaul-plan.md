...
- Current phase: Phase 0 - Preparation and Baseline
...
- **0.5b Creature Animation Pack:** full enemy and boss sprite sets with 4-direction movement, hit, and death/vulnerable states.
...
- Use a layered 2D scene: painted/baked background layers plus dynamic Phaser/Vue render layers.
...
- `public/assets/scene/` for board/background layers.
...
- `public/assets/ui/` for HUD/card frames, icons, and panel textures.
...
- [ ] Read `docs/design.md`, `docs/setting.md`, and `docs/plans/p0-implementation-plan.md`.
- [ ] Inspect current Phaser rendering in `RunScene.ts` and `runSceneRender.ts`.
- [ ] Inspect current Vue HUD/card surfaces in `RunHud.vue` and `RunHud.css`.
- [ ] Capture or select baseline screenshots for ready state, live combat, draft, boss, and result screens.
- [ ] Record current worktree status and identify unrelated pre-existing changes before editing.
- [ ] Confirm current commands still pass or record pre-existing failures: `npm run typecheck`, `npm test`, `npm run build`.
- [ ] Create the initial asset folder structure only when implementation begins.
- [ ] Define image generation prompt/spec files for style, board, tower, reaction, UI, enemy, and boss assets.

### Acceptance Criteria

- [ ] The visual baseline is documented with screenshots and notes.
- [ ] The implementer knows which files own Phaser render, Vue UI, and asset manifest concerns.
- [ ] Pre-existing failures or dirty worktree changes are recorded before visual work starts.
...
- [ ] Baseline screenshots are saved under ignored output or documented if already available.
- [ ] No P0 mechanics are changed in this phase.
...
- [ ] The asset pipeline supports later 4-direction creature animation.
...
## Phase 3 - Board, Куб, and Tower Visual Core
...
- [ ] Generate or produce the layered top-down scene background: cavern floor, fortress walls/edges, осадная галерея path, central Великий Перегонный Куб, gate/checkpoint, and atmospheric lighting.
...
Purpose: create approved source frames for every animated creature before generating animation strips.
...
- [ ] For each normal enemy, create 4-direction movement loops: up, down, left, right.
- [ ] For each normal enemy, create hit reaction animation.
- [ ] For each normal enemy, create death animation.
- [ ] For Бочкоед, create 4-direction crawl loops.
...
- [ ] Every P0 enemy has 4-direction move, hit, and death coverage.
...
- [ ] Select direction based on current path segment: up, down, left, or right.
...
- [ ] Direction changes look acceptable along the rectangular loop.
...
- [ ] Seven P0 enemy archetypes have 4-direction move, hit, and death animation coverage.
