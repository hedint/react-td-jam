...
- **T3 climax**: do not lock the role of `Огненный Шторм` before A5. The public demo needs one reliable final climax, but it may be either a reachable/juicy T3 or the boss Reaction Break. If boss Reaction Break carries the final wow, T3 must still be treated intentionally as rare aspirational content or adjusted; it must not remain accidentally dead content.
...
- [ ] Ensure `docs/plans/p06-finishing-and-ship-plan.md` is tracked as the primary ship roadmap.
...
- [ ] Update stale factual metadata in p06 before roadmap generation, including the current test count if it still says an older number.
- [ ] Add the detailed roadmap directly to this p06 document under each phase instead of creating a separate roadmap file, using phase/block-level steps rather than per-file implementation tasks.
...
#### Tasks
- [ ] Remove / flip `SKIP_UPGRADE_SELECTION_DRAFT_STEP` in `src/entities/game-session/model/draft.ts` so the upgrade draft step runs again.
- [ ] Add a configurable upgrade-draft eligibility hook in `draft.ts` (`createDraftState` / `advanceAfterDraft`); final cadence is decided in A4, not hardcoded during the technical re-enable.
- [ ] Reverify the upgrade draft UI in `src/widgets/run-hud/ui/RunHud.vue` (step has been dark): reroll, stack counts, descriptions render correctly.
- [ ] Add / update headless scenarios in `headlessRun.ts` so upgrade application is covered again.

#### Acceptance Criteria
- [ ] Picking a tower no longer auto-skips; upgrades apply and visibly affect reactions (coverage / slow / capacity / fire DPS).
- [ ] Upgrade screens can be enabled on a tunable cadence, with the shipping cadence deferred to the A4 wave-pacing decision.
- [ ] `npm run test` green.
...
#### Tasks
- [ ] Lock the 3 inner-corner junction slots at start (via `lockInnerCornerSlots` / `getInnerCornerCellIndexes` in `boardGeometry.ts`); the entrance/exit corner has no inner slot.
- [ ] Add an "unlock corner slot" upgrade kind to `config.upgrades` and `UpgradeId` (`types.ts`); each pick opens one specific corner; stop offering once all 3 are open.
- [ ] Offer slot unlocks through the normal upgrade pool first; do not make them guaranteed milestone rewards unless A4 proves the pool behavior hurts pacing.
- [ ] Add an `unlockSlot` transition in the `simulation.ts` action processor that flips `slot.locked`.
- [ ] Render locked slots distinctly and signal they are unlockable (`RunScene.ts` slot rendering / `runSceneTowerRender.ts`).

#### Acceptance Criteria
- [ ] All 3 corner slots start locked and reject placement.
- [ ] Choosing the unlock upgrade opens exactly one corner; the opened slot accepts a tower and projects into 2 cells.
- [ ] The option is skippable for an entire run with no errors; serialized save/resume preserves locked/unlocked state.
...
#### Tasks
- [ ] Build on the existing `?debug=1` HUD (`src/widgets/debug-hud/ui/DebugHud.vue`) and the unused `debugVisible` toggle (`simulation.ts`, `types.ts`).
- [ ] Add minimum A4-blocking controls: pin seed, skip to wave, jump to boss, lock core HP (god mode), and extra fast-forward beyond 2x.
- [ ] Add focused QA controls if still needed after the minimum loop: force-spawn enemy and force-spawn reaction.
- [ ] Route all controls through the action processor + `gameEvents` bus (`src/shared/lib/event-bus/gameEvents.ts`) so the sim stays authoritative.

#### Acceptance Criteria
- [ ] A full run can be exercised wave-by-wave (or jumped to boss) in seconds with a pinned seed.
- [ ] A4 mixed-wave balance work does not begin until the minimum A3 controls are implemented and verified behind `?debug=1`.
- [ ] Tooling has no effect when `?debug=1` is absent.
...
#### Tasks
- [ ] Audit the current 10-wave curve against real playthroughs and headless strategies: time per wave, leaks/kills, required placements, and whether upgrades make later choices trivial.
- [ ] Redesign the wave table so waves 1-3 remain onboarding waves with somewhat larger packs, while later waves use 2-3 monster roles per wave: flyers, one heavier or faster pressure unit, and scaling common "meat" for that wave.
- [ ] Account for the combat-system nuance that adding more enemies mostly stretches wave duration rather than linearly increasing required DPS; use mixed roles, HP/speed pressure, resistances, and timing overlaps to create decisions.
- [ ] Decide upgrade cadence only after the revised wave pressure is sketched: every N waves, milestone waves, alternating tower/upgrade, or another rule.
- [ ] Tie upgrade offerings and slot-unlock opportunities to the revised pressure curve so progression choices answer real upcoming problems instead of becoming free power.
- [ ] Check whether slot unlocks remain discoverable and timely as normal random upgrade-pool offers; if not, decide and document a milestone-offer fallback.
- [ ] Update `config.ts` wave definitions and `headlessRun.ts` scripted strategies once the curve is chosen.

#### Acceptance Criteria
- [ ] Waves 1-3 teach and stretch the run slightly without requiring deep optimization.
- [ ] Later non-boss waves regularly combine 2-3 enemy roles and force at least small placement/upgrade decisions.
- [ ] Upgrade cadence, upgrade strength, and slot-unlock timing are justified by the revised wave curve.
- [ ] The game can no longer be comfortably cleared by random upgrades and no meaningful tower placement after wave 3.

### A5 - Balance and wow-moment pass

#### Tasks
- [ ] Tune the chosen upgrade cadence, upgrade magnitudes, slot-unlock value, and revised wave difficulty in `config.ts`.
...
- [ ] Decide the T3 question (see Open Questions): make T3 reachable/juicy with any extra presentation it needs, or lean the final climax on boss Reaction Break while documenting T3 as rare aspirational content or adjusting it so it is not dead content.
...
#### Acceptance Criteria
- [ ] A competent run is winnable-but-tense; an underbuilt run can lose (headless weak/expected-win strategies still meaningful).
- [ ] The early electro-puddle melt and mid-run storm chain are reproducible in normal play; the final climax is reliable through either T3 `Огненный Шторм` or boss Reaction Break.
- [ ] If T3 is not the reliable final climax, its intended role is explicitly recorded as rare aspirational content or the reaction/progression is adjusted.
...
#### Tasks
- [ ] If A5 keeps T3 `fireStorm` as the reliable final climax, decide whether it needs a multi-cell pool underlay beyond the existing sprite VFX; add it only if the playtest/readability pass proves the extra layer is needed.
...
- **T3 climax**: make Огненный Шторм reachable and juicy, or rely on boss Reaction Break for the reliable final wow while keeping T3 intentionally rare/aspirational rather than accidentally dead. Decide during A5.
